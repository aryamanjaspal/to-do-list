...
def add_task():
    ...
    if Ntask != "":
        ...
        label_dsply["text"] = "please enter task"
    text_input.delete(0, 'end')


def delete_all():
    conf = messagebox.askquestion(
        'delet all??', 'are you sure to delete task?')
    print(conf)
    if conf.upper() == "YES": 
        global tasks
        tasks = []
...
label_title = tkinter.Label(root, text="Todo List", bg="dark blue",fg="white")
label_title.grid(row=0, column=0)

label_dsply = tkinter.Label(root, text="Tasks", bg="dark blue",fg="white")
...
text_add_bttn = tkinter.Button(
    root, text="ADD TASK", bg="white", fg="dark blue", width=20,height=2, command=add_task)
text_add_bttn.grid(row=1, column=0)

delone_bttn = tkinter.Button(
    root, text="DONE TASK", bg="white",fg="dark blue", width=20,height=2, command=delete_one)
delone_bttn.grid(row=2, column=0)

delall_bttn = tkinter.Button(
    root, text="DELETE ALL", bg="white",fg="dark blue", width=20,height=2, command=delete_all)
delall_bttn.grid(row=3, column=0)

sort_asc = tkinter.Button(root, text="sort(ASCENDING)",bg="white",fg="dark blue", width=20,height=2, command=sort_asc)
sort_asc.grid(row=4, column=0)

sort_dsc = tkinter.Button(root, text="sort(DESCENDING)",
                          bg="White",fg="dark blue", width=20,height=2, command=sort_dsc)
sort_dsc.grid(row=5, column=0)

random_bttn = tkinter.Button(
    root, text="RANDOM TASK", bg="White",fg="dark blue", width=20,height=2, command=random_task)
random_bttn.grid(row=6, column=0)

number_task = tkinter.Button(
    root, text="NUMBER OF TASK", bg="white",fg="dark blue", width=20,height=2, command=number_task)
number_task.grid(row=7, column=0)

exit_bttn = tkinter.Button(root, text="exit app",
                           bg="white",fg="dark blue" ,width=20,height=2, command=exit_app)
exit_bttn.grid(row=8, column=0)

save_button = tkinter.Button(
    root, text="save TODLIST", bg="white",fg="dark blue" ,width=20,height=2, command=save_act)
save_button.grid(row=10, column=1)

load_button = tkinter.Button(
    root, text="load TODOLIST", bg="white",fg="dark blue", width=20,height=2, command=load_act)
load_button.grid(row=10, column=0)

info_button = tkinter.Button(
    root, text="INFO", bg="white",fg="dark blue", width=20,height=2, command=load_info)
